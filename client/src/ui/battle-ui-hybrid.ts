/**
 * UI de Combate HÍBRIDA - Guardian Grove
 * Mantém TODA a lógica 2D + Arena 3D na área das bestas
 */
...
export class BattleUIHybrid {
  ...
  public dispose() {
    ...
    if (this.arenaScene3D) {
      this.arenaScene3D.dispose();
      this.arenaScene3D = null;
    }
    if (this.arenaScene3DContainer) {
      this.arenaScene3DContainer.remove();
      this.arenaScene3DContainer = null;
    }
    
    // Cleanup mini viewers (compatibilidade)
    if (this.playerViewer3D) {
